...
class ImageFilters():
    ...
    def __init__(self, camCal, projectedX, projectedY, defaultThrowDistance=100.0, debug=False):
        ...
        self.currentRoadEdgeProjected = np.zeros(
            (self.projectedY, self.projectedX, 3), np.uint8)

        # Current Projected Image
        self.currentRoadProjected = np.zeros(
    ...
    def projectionThrowDistanceDetect(self, debug=False, thresh=150.0):
        ...
        if not self.throwDistanceFound:
            maskedEdge = np.copy(self.currentRoadEdgeProjected[
                                 :, :, 1]).astype(np.uint8)
            topOfThrow = 0
            while not self.throwDistanceFound and topOfThrow < int(self.projectedY * 0.75):
    ...
    def setRoadProjection(self, projected):
        ...
        """
        self.currentRoadProjected = np.copy(projected)

    def getRoadProjection(self):
        """ Get the full road projection image
        """
        return self.currentRoadProjected
