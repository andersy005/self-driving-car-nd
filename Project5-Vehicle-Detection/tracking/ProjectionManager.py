...
class ProjectionManager():
    ...
    def set_image_filter(self, imgftr):
        ...
        self.currentImageFilter = imgftr
    ...
    def findInitialRoadCorners(self, imgftr):
        ...
        if lane_info[0] > -1000:
            ...
            self.currentDstRoadCorners = np.float32(
            ...
            self.current3DDstRoadCorners = np.zeros((4, 3), np.float32)
            self.current3DDstRoadCorners[:, :2] = self.currentDstRoadCorners
            ...
            projected, M = self.unwarp_lane(
                imgftr.currentImage, self.currentSrcRoadCorners, self.currentDstRoadCorners, self.mtx)
            imgftr.setRoadProjection(projected)

            self.lane_info = lane_info

            # calculate the rotation and translation vectors
            ret, self.rves, self.tvecs = cv2.solvePnP(self.current3DDstRoadCorners.reshape([4, 1, 3]),
                                                      self.current3DDstRoadCorners.reshape([4, 1, 2]), self.mtx, self.dist, flags=cv2.SOLVEPNP_ITERATIVE)
        ...
        if self.debug:
            ...
            if lane_info[0] > -1000:
                ...
                cv2.putText(
                    ...
                    (int(self.currentDstRoadCorners[0][0]),
                     int(self.currentDstRoadCorners[0][1]) - 1),
                    (int(self.currentDstRoadCorners[0][0]) - 275,
                     int(self.currentDstRoadCorners[0][1]) - 15),
                    font, 1, (255, 0, 0), 2)
                cv2.putText(
                    self.diag4, 'x2,y2: %d,%d' %
                    (int(self.currentDstRoadCorners[1][0]),
                     int(self.currentDstRoadCorners[1][1]) - 1),
                    (int(self.currentDstRoadCorners[1][0]) + 25,
                     int(self.currentDstRoadCorners[1][1]) - 15),
                    font, 1, (255, 0, 0), 2)
                cv2.putText(
                    self.diag4, 'x3,y3: %d,%d' %
                    (int(self.currentDstRoadCorners[2][0]),
                     int(self.currentDstRoadCorners[2][1]) - 1),
                    (int(self.currentDstRoadCorners[2][0]) + 25,
                     int(self.currentDstRoadCorners[2][1]) - 15),
                    font, 1, (255, 0, 0), 2)
                cv2.putText(
                    self.diag4, 'x4,y4: %d,%d' %
                    (int(self.currentDstRoadCorners[3][0]),
                     int(self.currentDstRoadCorners[3][1]) - 1),
                    (int(self.currentDstRoadCorners[3][0]) - 275,
                     int(self.currentDstRoadCorners[3][1]) - 15),
                    font, 1, (255, 0, 0), 2)
                ...
                cv2.circle(
                    self.diag4,
                    (int(self.currentDstRoadCorners[0][0]),
                     int(self.currentDstRoadCorners[0][1])),
                    10, (255, 64, 64), 10)
                cv2.circle(
                    self.diag4,
                    (int(self.currentDstRoadCorners[1][0]),
                     int(self.currentDstRoadCorners[1][1])),
                    10, (255, 64, 64), 10)
                cv2.circle(
                    self.diag4,
                    (int(self.currentDstRoadCorners[2][0]),
                     int(self.currentDstRoadCorners[2][1])),
                    10, (255, 64, 64), 10)
                cv2.circle(
                    self.diag4,
                    (int(self.currentDstRoadCorners[3][0]),
                     int(self.currentDstRoadCorners[3][1])),
                    10, (255, 64, 64), 10)
    ...
    def pixel2Meter(self):
        return self.currentImageFilter.throwDistance / self.projectedY
    ...
    def drawAxisOnLane(self, perspectiveImage):
        be_corner = np.float32(
            [[self.currentDstRoadCorners[0][0],
              self.currentDstRoadCorners[0][1], 0],
             [self.currentDstRoadCorners[0][0],
              self.currentDstRoadCorners[0][1], 0],
             [self.currentDstRoadCorners[0][0],
              self.currentDstRoadCorners[0][1], 0]]).reshape(-1, 3)

        be_axis = np.float32(
            [[self.currentDstRoadCorners[0][0] + 64,
              self.currentDstRoadCorners[0][1], 0],
             [self.currentDstRoadCorners[0][0],
              self.currentDstRoadCorners[0][1] + 128, 0],
             [self.currentDstRoadCorners[0][0],
              self.currentDstRoadCorners[0][1], -64]]).reshape(-1, 3)
    ...
    def drawCalibrationCube(self, perspectiveImage):
        be_cube = np.float32(
            [[self.currentDstRoadCorners[0][0],
              self.currentDstRoadCorners[0][1], 0],
             [self.currentDstRoadCorners[1][0],
              self.currentDstRoadCorners[1][1], 0],
             [self.currentDstRoadCorners[2][0],
              self.currentDstRoadCorners[2][1]-100, 0],
             [self.currentDstRoadCorners[3][0],
              self.currentDstRoadCorners[3][1]-100, 0],
             [self.currentDstRoadCorners[0][0],
              self.currentDstRoadCorners[0][1], self.z*2],
             [self.currentDstRoadCorners[1][0],
              self.currentDstRoadCorners[1][1], self.z*2],
             [self.currentDstRoadCorners[2][0],
              self.currentDstRoadCorners[2][1]-100, self.z*2],
             [self.currentDstRoadCorners[3][0],
              self.currentDstRoadCorners[3][1]-100, self.z*2]]).reshape(-1, 3)
